...
function setSlider(maxValue){
      ...
      jQuery("#amount").keydown(function(event) {
    	  var regex = new RegExp(/[0-9]+/);
    	    ...
    	    if (!regex.test(key)) {
    	    	if (key.charCodeAt(0) == 8 || key.charCodeAt(0) == 9 ||key.charCodeAt(0) == 46 || key.charCodeAt(0)==37 || key.charCodeAt(0) ==39) {
    	    			return true;
    	    			}
	...
	switch(status){
		...
		case "DEALER_BUSTED":
			if(bj){
				...
				if(countWins==0){
					swal("Well played! ", "You're paid 1:1 to your bid - "+data['winSum']/2);
				}
			if(countWins==1){
				swal("Wow! Second in a row! Your win is - "+data['winSum']/2,"Wanna try again, huh?");
			}
			if(countWins==2){
				swal("Come on! It's impossible! ","We bet you're cheating! Now you stole "+data['winSum']/2 +" from us!");
			}
			if(countWins>2){
		...
		case "PLAYER_WINS":
			if(bj){
				...
				if(countWins>2){
					swal("COME FREAKING ON! NO WAY!","You won "+data['winSum']/2);
				}
				countWins++;
				}
			break;
